"""
Inserção de fórmulas em planilha Excel com OpenPyXL.

Adiciona colunas calculadas (Preço de venda, Total, Lucro)
a uma planilha existente usando fórmulas de célula.
"""
...
# Índices das colunas (1-based: A=1, B=2, ...)
colum_produto = 1
colum_quantidade = 2
colum_preco = 3        # Preço de custo
colum_preco_venda = 4  # Preço de venda (custo * 1.3)
colum_total = 5        # Total = preço_venda * quantidade
colum_lucro = 6        # Lucro = total - (custo * quantidade)
...
# ============================================================
# INSERÇÃO DE FÓRMULAS LINHA A LINHA
# ============================================================
...
sheet.merge_cells(
    start_row=linha_total, start_column=colum_produto,
    end_row=linha_total, end_column=colum_quantidade
)

sheet.cell(row=linha_total, column=colum_lucro).value = (
    f'=SUM({get_column_letter(colum_lucro)}2:'
    f'{get_column_letter(colum_lucro)}{max_row})'
)
sheet.cell(row=linha_total, column=colum_preco).value = (
    f'=SUM({get_column_letter(colum_preco)}2:'
    f'{get_column_letter(colum_preco)}{max_row})'
)
...
print(f"Fórmulas inseridas em '{file_path}'.")